...
dat8 <- dat_l %>% filter(week == 8) %>% select(all_of(vars))
dat12 <- dat_l %>% filter(week == 12) %>% select(all_of(vars))


dat0_4 <- data.frame(dat0, dat4,
         select(dat_w, all_of(covs)))

dat4_8 <- data.frame(dat4, dat8,
                     select(dat_w, all_of(covs)))

dat8_12 <- data.frame(dat8, dat12,
                     select(dat_w, all_of(covs)))
...
for (i in 1:k) {
  set.seed(1)
  lassoreg <- cv.glmnet(data.matrix(dat0_4[,c(1:k,(k*2+1):(k*2+numCovar))]), dat0_4[,(k+i)],
...
for (i in 1:k) {
  set.seed(1)
  lassoreg <- cv.glmnet(data.matrix(dat4_8[,c(1:k,(k*2+1):(k*2+numCovar))]), dat4_8[,(k+i)],
                        family="gaussian", alpha=1, standardize=TRUE)
  lambda <- lassoreg$lambda.min
...
for (i in 1:k) {
  set.seed(1)
  lassoreg <- cv.glmnet(data.matrix(dat8_12[,c(1:k,(k*2+1):(k*2+numCovar))]), dat8_12[,(k+i)],
                        family="gaussian", alpha=1, standardize=TRUE)
  lambda <- lassoreg$lambda.min
...
nwc1 <- qgraph(adjMat2, nodeNames=names, legend.cex = .5, title = "baseline to week 4",
               threshold = .05, layout=L, asize=7, vsize=8, label.cex=1)

nwc2 <- qgraph(adjMat4, nodeNames=names, legend.cex = .5, title = "week 4 to week 8",
               threshold = .05, layout=L, asize=7, vsize=8, label.cex=1)

nwc3 <- qgraph(adjMat6, nodeNames=names, legend.cex = .5, title = "week 8 to week 12",
               threshold = .05, layout=L, asize=7, vsize=8, label.cex=1)

all_nwc <- list('baseline to w4'=nwc1,'w4 to w8'=nwc2,'w8 to w12'=nwc3)
